...
public func createCorrectTaskManager() -> PTNet {
    ...
    let processPool = PTPlace(named: "processPool")
    let inProgress  = PTPlace(named: "inProgress")
    let regulateur  = PTPlace(named: "regulateur")    //ajout de la place
    ...
    let success     = PTTransition(
        named          : "success",
        preconditions  : [PTArc(place: taskPool), PTArc(place: inProgress), PTArc(place: regulateur)],  
        postconditions : [])
    let exec       = PTTransition(
        named          : "exec",
        preconditions  : [PTArc(place: taskPool), PTArc(place: processPool)],
        postconditions : [PTArc(place: taskPool), PTArc(place: inProgress), PTArc(place: regulateur)])
    let fail        = PTTransition(
        named          : "fail",
        preconditions  : [PTArc(place: inProgress), PTArc(place: regulateur)],
        postconditions : [])

    // P/T-net
    return PTNet(
        places: [taskPool, processPool, inProgress, regulateur], //ajout de la place
        transitions: [create, spawn, success, exec, fail])
}
